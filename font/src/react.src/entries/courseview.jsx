...
import { getDone, markDone } from '../../js.src/api/progress'
import { getVideo } from '../../js.src/lib/videoDB'
...
function QuestionOverlay({ q, num, onClose }) {
  ...
  function handleSubmit() {
    ...
    if (q.isFechada) {
      ...
      setFeedback({ text: textAnswer.trim() ? 'Resposta registrada!' : 'Nenhuma resposta digitada.', type: 'neutral' })
  ...
  return (
    <div className="cv-q-overlay" style={{ display: 'flex' }}>
      <div className="cv-q-modal">
        ...
        {feedback && <div className={`cv-q-feedback ${feedback.type}`}>{feedback.text}</div>}
        {gabarito && (
...
function CourseViewApp() {
  ...
  if (notFound) {
    return (
      <div style={{ padding: '40px', color: '#e6edf3' }}>
        ...
        <a href="/font/pages/html.pages/user.html" style={{ color: '#00c8a0' }}>← Voltar</a>
      </div>
    )
  }
  ...
  return (
    <>
        ...
        <aside className="cv-right">
          <div className="cv-tabs">
            {[
              ...
              >
                {tab.label}
                {tab.badge !== undefined && <span className="cv-tab-badge">{tab.badge}</span>}
              </button>
            ))}
          </div>
          ...
          {activeTab === 'notes' && (
            <div className="cv-tab-panel">
              <div className="cv-notepad-top">
                <span className="cv-saved-indicator" style={{ opacity: savedIndicator ? 1 : 0 }}>salvo ✓</span>
              </div>
              <textarea
      ...
      {overlayData && (
        <QuestionOverlay q={overlayData.q} num={overlayData.num} onClose={handleOverlayClose} />
      )}
    </>
  )
}
