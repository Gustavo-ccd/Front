...
|---|---|---|
| 2026-06-18 | `font/pages/css.pages/landing.css` | **Hero ajustada em 900px**: topo voltou para linha única com auth à direita, hero recebeu foto em largura total com contraste mais suave e o bloco do PASCAL foi centralizado melhor, com os cards descendo um pouco para separar a composição. |
| 2026-06-18 | `font/pages/css.pages/landing.css` | **Hero mobile a partir de 900px**: a imagem lateral da hero passou a virar background com overlay já no breakpoint de 900px, e o `<img>` inline foi ocultado nessa faixa para manter a composição limpa em tablets e celulares. |
| 2026-06-18 | `font/pages/css.pages/landing.css` | **Hero mobile com foto de fundo**: na faixa estreita da landing, a imagem lateral da hero virou background com overlay para manter contraste, e o `<img>` inline foi ocultado no celular. |
| 2026-06-18 | `font/pages/js.pages/courseModals.js`, `font/src/js.src/api/courses.js` | **Resize de foto antes de salvar**: ao selecionar foto no modal "Novo curso", a imagem é redimensionada via canvas para no máximo 800×600px (JPEG 85%) antes de ser exibida no preview e salva no localStorage. `courses.js` atualizado para aceitar data URL string além de File no `create()`. |
| 2026-06-18 | `font/pages/css.pages/global.css`, `font/pages/css.pages/administrative.css`, `font/pages/css.pages/landing.css`, `font/src/react.src/entries/landing.jsx` | **Landing: catálogo igual ao dashboard**: estilos `dash-course-card` movidos de `administrative.css` para `global.css` (compartilhado). `landing.jsx` atualizado para usar o mesmo layout de cards com foto e nome sobreposto. Estilos antigos do catálogo removidos do `landing.css`. Padding da `.apresentacao` reduzido de 60px para 40px para eliminar espaço excessivo. |
| 2026-06-18 | `font/pages/css.pages/course.css` | **Refinamento mobile da tela de cursos**: removidos breakpoints baseados em `zoom` e substituídos por um layout mais legível no celular. A tabela virou cards compactos com rótulos contextuais, ações empilhadas quando necessário e modais/forms com paddings e botões adaptados. |
| 2026-06-18 | `font/pages/css.pages/global.css`, `font/pages/css.pages/landing.css`, `font/pages/css.pages/user.css`, `font/pages/css.pages/administrative.css`, `font/pages/css.pages/courseView.css` | **Ajuste de mobile**: removidos breakpoints baseados em `zoom` e substituídos por responsividade real. Landing, dashboards e aula agora empilham melhor em telas pequenas, com paddings menores, grids mais simples, sidebar adaptada e overlay/modal ajustados para toque e leitura no celular. |
